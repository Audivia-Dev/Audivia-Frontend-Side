...
export const TourMap: React.FC<UserLocationMapProps> = ({ tour, height = 300 }) => {
    ...
    const fetchRoute = useCallback(async (checkpoints: Checkpoint[]) => {
        ...
        const cacheKey = `ROUTE_CACHE_${tour.id}`;
        try {
            ...
            const payload = {
                origin: { location: { latLng: { latitude: checkpoints[0].latitude, longitude: checkpoints[0].longitude } } },
                destination: { location: { latLng: { latitude: checkpoints[checkpoints.length - 1].latitude, longitude: checkpoints[checkpoints.length - 1].longitude } } },
                intermediates: checkpoints.slice(1, -1).map((cp: Checkpoint) => ({ location: { latLng: { latitude: cp.latitude, longitude: cp.longitude } } })),
                travelMode: "WALK",
            ...
            const response = await fetch(url, { method: "POST", headers: prepareHeaders, body: JSON.stringify(payload) });
            ...
            if (json.routes && json.routes.length > 0) {
                ...
                const totalDuration = route.duration ? parseInt(route.duration.replace("s", "")) / 60 : 0;
                const newRouteInfo = { distance: totalDistance.toFixed(2), duration: totalDuration.toFixed(0) };

                setRouteCoordinates(points);
                setRouteInfo(newRouteInfo);

                await AsyncStorage.setItem(cacheKey, JSON.stringify({ routeCoordinates: points, routeInfo: newRouteInfo }));
